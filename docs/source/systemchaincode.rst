...
udo related queries and VSCC (Validation System Chaincode) for validating
...
Every system chaincode must implement the `Chaincode Interface <http://godoc.org/github.com/hyperledger/udo/core/chaincode/shim#Chaincode>`_
...
.. note:: On imported packages: the Go standard library requires that a plugin must
          include the same version of imported packages as the host application (udo, in this case)
