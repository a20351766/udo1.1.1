...
A client for exercising this chaincode is avilable at https://github.com/srderson/hyperledger-udo-utxo-client-java.


The following are instructions for building and deploying the UTXO chaincode in Hypereledger Udo. All commands should be run with vagrant.

First, build the Docker image for the UTXO chaincode.

```
cd $GOPATH/src/github.com/hyperledger/udo/examples/chaincode/go/utxo/
docker build -t utxo:0.1.0 .
```

Next, modify the `core.yaml` file in the Hyperledger Udo project to point to the local Docker image that was built in the previous step. In the core.yaml file find `chaincode.golang.Dockerfile` and change it from from `hyperledger/udo-baseimage` to `utxo:0.1.0`
...
```
CORE_PEER_ADDRESS=localhost:7051 peer chaincode deploy -p github.com/hyperledger/udo/examples/chaincode/go/utxo -c '{"Function":"init", "Args": []}'
```
...
```
go test github.com/hyperledger/udo/core/container -run=BuildImage_Peer
```

Using the Docker image that we just built, start a peer within a container in `chaincodedev` mode.
```
docker run -it -p 7051:7051 -p 7053:7053 hyperledger/udo-peer peer node start --peer-chaincodedev
```
