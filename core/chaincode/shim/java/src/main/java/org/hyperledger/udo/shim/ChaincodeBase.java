...
package org.hyperledger.udo.shim;

import static org.hyperledger.udo.shim.Chaincode.Response.Status.INTERNAL_SERVER_ERROR;
import static org.hyperledger.udo.shim.Chaincode.Response.Status.SUCCESS;
...
import org.hyperledger.udo.protos.peer.ChaincodeSupportGrpc;
import org.hyperledger.udo.protos.peer.ChaincodeSupportGrpc.ChaincodeSupportStub;
import org.hyperledger.udo.shim.impl.Handler;
import org.hyperledger.udo.shim.impl.NextStateInfo;
...
public abstract class ChaincodeBase implements Chaincode {
	...
	private String rootCertFile = "/etc/hyperledger/udo/peer.crt";
	...
	public void chatWithPeer(ManagedChannel connection) {
		...
		try {
			requestObserver = stub.register(new StreamObserver<ChaincodeMessage>() {
				...
				public void onNext(ChaincodeMessage message) {
					...
					try {
						logger.debug(String.format("[%-8s]Received message %s from org.hyperledger.udo.shim", message.getTxid(), message.getType()));
		...
		// Create the org.hyperledger.udo.shim handler responsible for all
		// control logic
		...
		while (true) {
			try {
				...
				handler.handleMessage(message);
				// keepalive messages are PONGs to the udo's PINGs
