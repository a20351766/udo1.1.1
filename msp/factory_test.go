...
func TestNew(t *testing.T) {
	...
	assert.Equal(t, runtime.FuncForPC(reflect.ValueOf(i.(*bccspmsp).internalSetupFunc).Pointer()).Name(), "github.com/hyperledger/udo/msp.(*bccspmsp).(github.com/hyperledger/udo/msp.setupV1)-fm")
	assert.Equal(t, runtime.FuncForPC(reflect.ValueOf(i.(*bccspmsp).internalValidateIdentityOusFunc).Pointer()).Name(), "github.com/hyperledger/udo/msp.(*bccspmsp).(github.com/hyperledger/udo/msp.validateIdentityOUsV1)-fm")

	i, err = New(&BCCSPNewOpts{NewBaseOpts{Version: MSPv1_1}})
	assert.NoError(t, err)
	assert.NotNil(t, i)
	assert.Equal(t, MSPVersion(MSPv1_1), i.(*bccspmsp).version)
	assert.Equal(t, runtime.FuncForPC(reflect.ValueOf(i.(*bccspmsp).internalSetupFunc).Pointer()).Name(), "github.com/hyperledger/udo/msp.(*bccspmsp).(github.com/hyperledger/udo/msp.setupV11)-fm")
	assert.Equal(t, runtime.FuncForPC(reflect.ValueOf(i.(*bccspmsp).internalValidateIdentityOusFunc).Pointer()).Name(), "github.com/hyperledger/udo/msp.(*bccspmsp).(github.com/hyperledger/udo/msp.validateIdentityOUsV11)-fm")
