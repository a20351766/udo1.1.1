Behave tests for Hyperledger Udo Feature and System Tests
============================================================
...
This drectory contains a behave implementation of system and feature file testing for Hyperledger Udo.
...
.. _hyperledger-udo: https://github.com/hyperledger/udo
...
BDD tests are testing functionality and feature behavior. With this in mind, the following are areas that we plan to be covered in these BDD tests:
   ...
   * Udo-CA (used for SSL connections)
...
The docker images for ``peer``, ``orderer``, ``kafka``, and ``zookeeper`` are needed. You must perform a ``make docker`` in the ``/path/to/hyperledger/udo`` directory.


=========================
Building tool executables
=========================
The **configtxgen** and **cryptogen** tools are used when bootstrapping the networks in these tests. As a result, you must perform a ``make configtxgen && make cryptogen`` in the ``/path/to/hyperledger/udo`` directory. Be sure that the executable location is added to your PATH.
...
-----------------------
   ...
   * Remove all images except for hyperledger/udo-baseimage
      * ``$ docker rmi $(docker images | grep -v 'hyperledger/udo-baseimage:latest' | awk {'print $3'})``
   * Start a container
