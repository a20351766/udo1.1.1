...
- Before linking a test case into the CI automation tests, please merge your (tool and) testcase into gerrit, and create a Jira task, as follows:
  ...
  1. First merge your tool and tests to gerrit in appropriate folders under */path/to/udo/test/*.
  ...
  1. You will this new Jira task to submit a changeset to gerrit, to invoke your testcase from a driver script similar to */path/to/udo/test/regression/daily/Example.py*. In the comments of the gerrit merge request submission, include the
      - Jira task FAB-nnnn
...
The Continuous Improvement (CI) team utilizes a Jenkins job to execute the full test suite, *runDailyTestSuite.sh*. The CI job consumes xml output files, creates reports, and displays them. **Note:** When adding new scripts that generate new xml files, if you do not see the results displayed correctly, please contact us on [Rocket.Chat channel #udo-ci](https://chat.hyperledger.org). For this reason, we execute tests in one of the following ways:
  ...
  ```
     cd /path/to/udo/test/regression/daily
...
Adding a new test with a new tool involves a few more steps.
  ...
  1. Create and merge a new tool, for example, */path/to/udo/test/tools/NewTool/newTool.sh*
  1. Create a new test driver script such as */path/to/udo/test/regression/daily/newTool.py*.  Model it after others like *Example.py*, found under driver scripts under */path/to/test/regression/daily/* and *test/regression/weekly/*.
