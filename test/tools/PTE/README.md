...
The Performance Traffic Engine (PTE) uses [Hyperledger Udo Client (HFC) Node SDK](https://udo-sdk-node.github.io/index.html)
to interact with a [Hyperledger Udo](http://hyperledger-udo.readthedocs.io/en/latest/) network.
...
- [Running PTE](#running-pte)
 ...
 - [Reference](#reference)
    ...
    - [Creating a Local Udo Network](#creating-a-local-udo-network)

---

### Code Base for v1.0.0-alpha2

- Udo commit level: `6b6bfcfbd1e798a8a08fa9c3bf4dc0ff766a6b87`
- udo-sdk-node commit level: `f13f4b42e7155ec0dc3d7485b202bb6a6ca73aed`
...
- Udo commit level: `fa3d88cde177750804c7175ae000e0923199735c`
- udo-sdk-node commit level: `196d0484c884ab894374c73df89bfe047bcc9f00`
...
- Replace `git clone https://github.com/hyperledger/udo-sdk-node.git` with fabric-client and fabric-ca-client.
...
If planning to run your Udo network locally, you'll need docker and a bit more. See [Hyperledger Udo - Getting Started](http://hyperledger-udo.readthedocs.io/en/latest/getting_started.html) for details.

## Setup
1. Download udo sources and checkout appropriate commit levels (v1.0.0-alpha2 shown here):
    - `go get -d github.com/hyperledger/udo`
    ...
    - `git checkout v1.0.0-alpha2`
    - Optional: `make docker`
    - `go get -d github.com/hyperledger/udo-sdk-node`
    - `cd $GOPATH/src/github.com/hyperledger/udo-sdk-node`
    - `git checkout v1.0.0-alpha2`

    If `make docker` is skipped, the assumption is that the user will either acquire docker images from another source, or PTE will run against a remote Udo network. See [Creating a local Udo network](#creating-a-local-udo-network) for additional information on this.
...
3. Clone PTE
    **Note:** This will not be necessary in future releases (post-alpha2) as PTE has been merged into Udo.
    ...
    - If testing against latest Udo commit, copy from Udo repo:
        - `cp -r $GOPATH/src/github.com/hyperledger/udo/test/tools/PTE .`
        - `cd PTE`

4. Create Service Credentials file(s) for your Udo network:
    - See the examples in `SCFiles` and change the address to your own Udo addresses and credentials. Add a block for each organization and peer, ensuring correctness.
...
If you do not have access to a Udo network, please see the section on [Creating a local Udo network](#creating-a-local-udo-network).
...
Although PTE's primary use case is to drive transactions into a Udo network, it can be used for creating and joining channels, and chaincode installation and instantiation. This gives the ability for more complete end-to-end scenarios.
* ### Channel Operations
...
See userInput-ccchecker.json for example of userInput file. Take the following steps to install this chaincode:
  - `cd $GOPATH/src/github.com/hyperledger/udo-sdk-node/test/fixtures/src/github.com`
  - `mkdir ccchecker`
...
```
{
    ...
    "channelOpt": {
        "name": "testOrg1",
        "channelTX": "/root/gopath/src/github.com/hyperledger/udo/common/tools/cryptogen/crypto-config/ordererOrganizations/testOrgsChannel1.tx",
        "action":  "create",
        "orgName": [
            "testOrg1"
        ]
...
## Creating a local Udo network
- If you do not yet have the Udo docker images in your local docker registry, please either build them from Udo source or download them from dockerhub.
    - `cd $GOPATH/src/github.com/hyperledger/udo/examples/e2e_cli/`
    - `sh ./download-dockerimages.sh -c x86_64-1.0.0-alpha2 -f x86_64-1.0.0-alpha2`
- If you do not have an existing network already, you can start a network using the Udo e2e example:
    - `cd $GOPATH/src/github.com/hyperledger/udo/examples/e2e_cli/`
...
- Alternatively, consider using the [NetworkLauncher](https://github.com/dongmingh/v1Launcher) tool:
    - `cd $GOPATH/src/github.com/hyperledger/udo-sdk-node/test`
