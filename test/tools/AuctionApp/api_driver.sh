...
LOG_FILE="$GOPATH/src/github.com/hyperledger/udo/test/envsetup/logs/auction_logs.log"
TEMP_LOG_FILE="temp_auction_logs.log"
ORDERER_IP=orderer.example.com:7050
LOG_LEVEL="error"

ORDERER_CA=$GOPATH/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
...
function setGlobals() {
	...
	if [ $1 -eq 0 -o $1 -eq 1 ]; then
		...
		CORE_PEER_TLS_ROOTCERT_FILE=$GOPATH/src/github.com/hyperledger/udo/peer/crypto/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt
		...
		CORE_PEER_TLS_ROOTCERT_FILE=$GOPATH/src/github.com/hyperledger/udo/peer/crypto/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt
		CORE_PEER_MSPCONFIGPATH=$GOPATH/src/github.com/hyperledger/udo/peer/crypto/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
		if [ $1 -eq 2 ]; then
...
createChannel() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer channel create -o $ORDERER_IP -c $CHANNEL_NAME$CH_NUM -f $GOPATH/src/github.com/hyperledger/udo/peer/channel-artifacts/channel$CH_NUM.tx -t 10 >&$LOG_FILE
		wait 5
	else
		peer channel create -o $ORDERER_IP -c $CHANNEL_NAME$CH_NUM -f $GOPATH/src/github.com/hyperledger/udo/peer/channel-artifacts/channel$CH_NUM.tx --tls --cafile $ORDERER_CA -t 10 >&$LOG_FILE
		wait 5
	fi
...
updateAnchorPeers() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer channel create -o $ORDERER_IP -c $CHANNEL_NAME$CH_NUM -f $GOPATH/src/github.com/hyperledger/udo/peer/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors$CH_NUM.tx >>$LOG_FILE
	else
		peer channel create -o $ORDERER_IP -c $CHANNEL_NAME$CH_NUM -f $GOPATH/src/github.com/hyperledger/udo/peer/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors$CH_NUM.tx --tls --cafile $ORDERER_CA >>$LOG_FILE
	fi
...
installChaincode() {
	for ((i = 0; $i < $ENDORSERS; i++)); do
		for ((ch = 0; $ch < $CHAINCODES; ch++)); do
			PEER=$i
			setGlobals $PEER
			peer chaincode install -n $CHAINCODE_NAME$ch -v 1 -p github.com/hyperledger/udo/test/chaincodes/AuctionApp >>$LOG_FILE
